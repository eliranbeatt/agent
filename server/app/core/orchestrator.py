...
class WorkflowMatcher:
    """Handles matching user requests to predefined workflows with advanced confidence scoring."""
    ...
    def __init__(self, workflows: Dict[str, WorkflowConfig]):
        ...
        self.workflow_patterns = {
            "rag_qa": ["question", "ask", "what", "how", "why", "explain", "tell me about"],
            "summarize": ["summarize", "summary", "brief", "overview", "key points"],
            "compare": ["compare", "difference", "versus", "vs", "contrast"],
            "extract": ["extract", "find", "get", "retrieve", "pull out"],
            "analyze": ["analyze", "analysis", "examine", "study", "investigate"]
        }
        
    def match_workflow(self, user_request: str) -> tuple[Optional[str], float]:
        """
        Match user request to the best workflow using advanced confidence algorithm.
        ...
        for workflow_name, workflow_config in self.workflows.items():
            ...
            confidence = self._calculate_confidence(request_lower, request_words, workflow_config)
    ...
    def _calculate_confidence(self, request: str, request_words: set, workflow: WorkflowConfig) -> float:
        """
        ...
        context_score = self._calculate_context_score(request, workflow)
        confidence += context_score * 0.1
        
        # Apply workflow-specific confidence threshold
        if hasattr(workflow, 'confidence_threshold'):
            confidence *= (1.0 + workflow.confidence_threshold) / 2.0
    ...
    def _calculate_trigger_score(self, request: str, triggers: List[str]) -> float:
        ...
        for trigger in triggers:
            ...
            if trigger_lower in request:
                matches += 1
            elif any(word in request for word in trigger_lower.split()):
                partial_matches += 0.5
    ...
    def _calculate_semantic_score(self, request: str, workflow: WorkflowConfig) -> float:
        ...
        # Simple semantic matching based on description
    ...
    def _calculate_context_score(self, request: str, workflow: WorkflowConfig) -> float:
        """Calculate context relevance score."""
        # Check for file-related context
        file_indicators = ["file", "document", "pdf", "upload", "image"]
        ...
        if "rag" in workflow.name.lower() or "qa" in workflow.name.lower():
            return 0.8 if has_file_context else 0.2
            
        # Summarize workflows also benefit from file context
        if "summarize" in workflow.name.lower():
            return 0.7 if has_file_context else 0.3
            
        return 0.5  # Neutral score for other workflows
